...
const cancelBooking = async (bookingID, userRole) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if ('message' in bookingData) {
        return reject(new Error(bookingData.message));
      }
      const response = {
        id: bookingID,
        refund: null,
      ...
      if (userRole === 'vendor') {
        ...
        if (bookingData.booking_status === 1) {
          ...
          return reject(new Error("Booking already cancelled!"));
        }
      ...
      if (cancelBooking) {
        ...
        if ('message' in booking) {
          return reject(new Error(booking.message));
        }
        response.status = 2;
      }
      let tot = 0
...
app.post('/cleanBookings', async (req, res, next) => {
  try {
    ...
    const GET_DATA_MUTATION = `query {
        bookings(where: {_and: {payment_status: {_in: [failed, processing]}, booking_status: {_eq: 0}}}) {
          updated_at
          booking_status
          reservation_date
          payment_status
    ...
    const bookingData = await require('axios')({
      ...
      .then(resp => {
        ...
        return resp.data.data.bookings;
      })
      .catch(err => {
        return {
          message: err.message
    ...
    for (let booking of bookingData) {
      const timeDiff = moment.duration(moment().diff(moment(booking.updated_at))).asMinutes()
      if (Math.abs(timeDiff) >= 15) {
        console.log(booking.id)
        await cancelBooking(booking.id, 'admin').catch(err => {
          return res.status(401).json({
            message: err.message
          });
        })
